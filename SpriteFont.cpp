...
namespace Dengine {
    ...
    void SpriteFont::init(const char* font, int size, char cs, char ce) {
        ...
        _regStart = cs;
        _regLength = ce - cs + 1;
        int padding = std::max(size / 8, 1);

        // First neasure all the regions
        glm::ivec4* glyphRects = new glm::ivec4[_regLength];
        int i = 0, advance;
        for (char c = cs; c <= ce; c++) {
            int err = TTF_GlyphMetrics(f, c, &glyphRects[i].x, &glyphRects[i].z, &glyphRects[i].y, &glyphRects[i].w, &advance);
            if (err == -1) fatalError("Couldn't get glyph metrics");
        ...
        for (int ri = 0; ri < bestRows; ri++) {
            ...
            for (size_t ci = 0; ci < bestPartition[ri].size(); ci++) {
                ...
                for (int i = 0; i < cp; i += 4) {
                    ...
                    sp[i + 1] = sp[i];     // Bug here!
                    sp[i + 2] = sp[i];     // Bug here!
                    //sp[i + 1] = (unsigned char)((float)sp[i+1] * a);
                    //sp[i + 2] = (unsigned char)((float)sp[i+2] * a);
                }
